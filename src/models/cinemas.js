...
exports.updateCinemas = async (id, data, cb) => {
  try {
    const sql =
      'UPDATE "cinemas" SET "picture" = COALESCE(NULLIF($1, \'\'), "picture"), "name" = COALESCE(NULLIF($2, \'\'), "name"), "address" = COALESCE(NULLIF($3, \'\'), "address"), "city" = COALESCE(NULLIF($4, \'\'), "city") WHERE id = $5 RETURNING *'
