...
exports.updateMovieSchedules = (id, data, cb) => {
  const sql =
    'UPDATE "movieSchedules" SET "movieId" = COALESCE(NULLIF($1,\'\')::INTEGER, "movieId"), "cinemaId" = COALESCE(NULLIF($2,\'\')::INTEGER, "cinemaId"), "price" = COALESCE(NULLIF($3,\'\')::INTEGER, "price"), "startDate" = COALESCE(NULLIF($4,\'\')::DATE, "startDate"), "endDate" = COALESCE(NULLIF($5,\'\')::DATE, "endDate") WHERE id = $6 RETURNING *'
  const values = [data.movieId, data.cinemaId, data.price, data.startDate, data.endDate, id]
  return poolString.query(sql, values, cb)
}
