...
exports.createReserveSeats = (data, cb) => {
  const sql = 'INSERT INTO "reserveSeats" ("seatNum", "transactionId") VALUES ($1,$2) RETURNING *'
...
exports.updateReserveSeats = (data, cb) => {
  const sql =
    'UPDATE "reserveSeats" SET "seatNum" = COALESCE(NULLIF($1,\'\'), "seatNum"), "transactionId" = COALESCE(NULLIF($2, \'\')::INTEGER, "transactionId") WHERE id = $3 RETURNING *'
  const values = [data.seatNum, data.transactionId, data.id]
  return poolString.query(sql, values, cb)
}
